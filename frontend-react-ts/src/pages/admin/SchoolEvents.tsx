...
const SchoolEvents = () => {
  ...
  // Final date modal state
  const [finalDateModal, setFinalDateModal] = useState<Approval | null>(null);
  const [finalDateValue, setFinalDateValue] = useState("");
  const [settingFinalDate, setSettingFinalDate] = useState(false);

  const backendUrl = "http://127.0.0.1:8000";

  // Since this component is admin-only, we don't need to check user type
  ...
  // Check if final date can be set (event approved - admin can always edit final date)
  const canSetFinalDate = (approval: Approval) => {
    return approval.event.status === "approved";
  };
  ...
  // Handle setting final date
  const handleSetFinalDate = async () => {
    if (!finalDateModal || !finalDateValue) return;

    try {
      setSettingFinalDate(true);
      await setFinalDate(finalDateModal.event.id, finalDateValue);

      // Refresh history to show updated final date
      await fetchApprovalHistory();

      // Close modal and reset
      setFinalDateModal(null);
      setFinalDateValue("");

      alert(
        finalDateModal.event.final_date
          ? "Final date updated successfully!"
          : "Final date set successfully!"
      );
    } catch (err: any) {
      console.error("Error setting final date:", err);
      if (err.response?.data?.message) {
        alert(err.response.data.message);
      } else {
        alert("Error setting final date. Please try again.");
      }
    } finally {
      setSettingFinalDate(false);
    }
  };
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex">
                        <button className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-indigo-600 hover:text-indigo-600 transition cursor-pointer">
                          <Paperclip
                            size={17}
                        ...
                        <button
                          className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-indigo-600 hover:text-indigo-600 transition cursor-pointer"
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-1">
                        {/* Final Date Button - show for approved events */}
                        {canSetFinalDate(approval) && (
                          <button
                            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-blue-100 text-blue-600 hover:text-blue-700 cursor-pointer transition"
                            onClick={() => {
                              setFinalDateModal(approval);
                              setFinalDateValue(
                    ...
                    <div className="text-sm text-gray-900 space-y-1 flex-1">
                      <p className="flex items-center">
                        <span className="font-medium">Org:</span>{" "}
                        <span
                          className="truncate max-w-[200px] ml-1"
                          title={event.student_org?.name}
                        >
                          {event.student_org?.name || "N/A"}
                        </span>
                      </p>
                      ...
                      {event.optional_date && (
                        <p>
                          <span className="font-medium">Optional:</span>{" "}
                          {formatDate(event.optional_date)}
                        </p>
                      )}
                      {event.final_date && (
          ...
          <div className="bg-white rounded shadow-lg w-full max-w-md p-6">
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-2">
                Event:{" "}
                <span className="font-medium">{finalDateModal.event.name}</span>
              </p>
              <p className="text-sm text-gray-600 mb-4">
                Organization:{" "}
                <span className="font-medium">
                  {finalDateModal.event.student_org?.name || "N/A"}
                </span>
              </p>
              ...
              <div className="space-y-2">
                <p className="text-sm">
                  ...
                  {formatDate(finalDateModal.event.proposed_date)}
                </p>
                {finalDateModal.event.optional_date && (
                  <p className="text-sm">
                    <span className="font-medium">Optional Date:</span>{" "}
                    {formatDate(finalDateModal.event.optional_date)}
                  </p>
                )}
                {finalDateModal.event.final_date && (
                  <p className="text-sm text-green-700">
                    <span className="font-medium">Current Final Date:</span>{" "}
                    {formatDate(finalDateModal.event.final_date)}
                  </p>
                )}
              </div>
            </div>
