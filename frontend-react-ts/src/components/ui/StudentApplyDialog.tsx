...
interface StudentApplyDialogProps {
  ...
  onCancel: () => void;
  onSubmit: (formData: { name: string; email: string; file: File }) => void | Promise<void>;
}
...
const StudentApplyDialog: React.FC<StudentApplyDialogProps> = ({
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const nameInput = (e.currentTarget.elements.namedItem("userName") as HTMLInputElement)?.value;
    const emailInput = (e.currentTarget.elements.namedItem("userEmail") as HTMLInputElement)?.value;

    if (nameInput && emailInput && selectedFile) {
      try {
        setSubmitting(true);
        await onSubmit({ name: nameInput, email: emailInput, file: selectedFile });
      } finally {
        setSubmitting(false);
      }
    }
